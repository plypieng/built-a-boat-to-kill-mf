...
@export var acoustic_material : Resource :
	set(value):
...
static func find_on(node: Node) -> Node:
	if node == null:
		return null
	for child in node.get_children():
		if child != null and child.is_class("AcousticBody"):
			return child
	return null
...
static func find_for_collider(collider: Node) -> Node:
	if collider == null:
		return null
