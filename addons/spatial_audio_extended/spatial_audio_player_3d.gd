...
func _ready() -> void:
	...
	effects_enabled = not global_effects_disabled
...
func _update_omni_distance(ray: RayCast3D, idx: int) -> void:
	...
	var ab := ACOUSTIC_BODY_SCRIPT.find_for_collider(collider)
	...
	for _bounce in fibonacci_ray_reflections:
		...
		var bounce_ab := ACOUSTIC_BODY_SCRIPT.find_for_collider(bounce_collider)
...
func _update_lowpass(listener: Node3D) -> void:
	...
	for _hit_idx in max_occlusion_hits:
		...
		var acoustic_body := ACOUSTIC_BODY_SCRIPT.find_for_collider(collider)
