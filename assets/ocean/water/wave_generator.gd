...
func init_gpu(num_cascades : int) -> void:
	...
	# Add to init_gpu() texture creation
	descriptors[&'displacement_map'] = context.create_texture(
		...
		RenderingDevice.TEXTURE_USAGE_CAN_COPY_FROM_BIT, # Critical flag
		num_cascades
		)
	...
	var spectrum_modulate_spectrum_set := context.create_descriptor_set([descriptors[&'spectrum']], spectrum_modulate_shader, 0)
	...
	var spectrum_modulate_fft_buffer_set := context.create_descriptor_set([descriptors[&'fft_buffer']], spectrum_modulate_shader, 1)
	var fft_unpack_images_set := context.create_descriptor_set([descriptors[&'displacement_map'], descriptors[&'normal_map']], fft_unpack_shader, 0)
	var fft_unpack_fft_buffer_set := context.create_descriptor_set([descriptors[&'fft_buffer']], fft_unpack_shader, 1)
	...
	pipelines[&'spectrum_modulate'] = context.create_pipeline([map_size/16, map_size/16, 1], [spectrum_modulate_spectrum_set, spectrum_modulate_fft_buffer_set], spectrum_modulate_shader)
	...
	pipelines[&'fft_unpack'] = context.create_pipeline([map_size/16, map_size/16, 1], [fft_unpack_images_set, fft_unpack_fft_buffer_set], fft_unpack_shader)
...
## Returns the displacement map texture RID for a specific cascade
